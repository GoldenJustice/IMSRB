...
export const load = (async ({locals}) => {
    ...
    const [units, brigades] = await Promise.all([
        locals.pb.collection('units').getFullList<UnitsResponse>({
            filter: `"${user?.brigade}" ~ brigadeID:each`,
            expand: 'brigadeID'
        }),
        locals.pb.collection('Brigades').getFullList<BrigadesResponse>({
    ...
    console.log("Incident Layout reload")
