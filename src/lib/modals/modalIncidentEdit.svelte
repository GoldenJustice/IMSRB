<script lang="ts">
    import { IncidentsStatusOptions, type BrigadesResponse, type UnitsResponse } from '$lib/algemeen/pocketbase-types';
  ...
  let {data, children} = $props();
	...
	// Form Data
	let formData = $modalStore[0].meta.userdata.incident;
...
{#if $modalStore[0]}
	...
	<div class="incident-modal">
      ...
      {#if hasPermission($modalStore[0].meta.userdata.user, $modalStore[0].meta.userdata.gebruikerRol, Permissions.INCIDENTEN.SLUITEN.ALLE)}
    <button class="btn btn-error" onclick={() => {formData.Status = IncidentsStatusOptions.Afgerond; onFormSubmit()}}>Incident Afronden</button>
    {/if}
